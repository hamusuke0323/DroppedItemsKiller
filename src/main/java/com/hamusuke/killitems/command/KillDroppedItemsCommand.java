...
@Command(label = "killdroppeditems", aliases = {"kdi"})
public final class KillDroppedItemsCommand implements CommandHandler {
    ...
    public void execute(Player sender, Player targetPlayer, List<String> args) {
        ...
        if (items.size() == 0) {
            CommandHandler.sendMessage(sender, Language.translate(sender, "cmd.kdi.none"));
            return;
        }

        CommandHandler.sendMessage(sender, Language.translate(sender, "cmd.kdi.success", items.size()));
    }
}
